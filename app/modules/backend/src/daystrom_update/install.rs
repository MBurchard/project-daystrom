...
const RELEASE_OWNER: &str = "MBurchard";

/// GitHub repository trusted to publish production updater packages.
const RELEASE_REPOSITORY: &str = "project-daystrom";
...
pub async fn install_daystrom_update(app: tauri::AppHandle) {
    ...
    if let Err(error) = validate_download_url(&update.download_url) {
        log_warn!("{error}");
...
fn validate_download_url(url: &tauri::Url) -> Result<(), String> {
    if debug_endpoint_active() || is_trusted_release_download_url(url) {
        return Ok(());
    }
...
fn is_trusted_release_download_url(url: &tauri::Url) -> bool {
    if url.scheme() != "https"
...
mod tests {
    ...
    fn production_download_url_requires_trusted_github_repository() {
        ...
        assert!(is_trusted_release_download_url(&trusted));
        assert!(!is_trusted_release_download_url(&wrong_repository));
        assert!(!is_trusted_release_download_url(&insecure));
        assert!(!is_trusted_release_download_url(&unrelated_path));
        assert!(!is_trusted_release_download_url(&query));
    }
